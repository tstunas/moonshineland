  ...
  function generateSpaceFromAH(a: number, h: number) {
    if (a === 0 && h === 1) {
      return HALF_SPACE;
    }
    if (a < h || a < 0 || h < 0) {
      throw new Error("a:" + String(a) + ",h:" + String(h));
    }
    return (
      DOTS_TO_SPACE[11].str.repeat(a - h) +
      (DOTS_TO_SPACE[11].str + HALF_SPACE).repeat(h)
    );
  }

  function adjustWithUnicode(adjuster: number) {
    if (adjuster >= 11) {
      throw new Error("Adjuster is " + String(adjuster));
    }
    if (adjuster === 0) {
      return "";
    }
  ...
  function oneDotReduce(ah: { a: number; h: number; adj: number }) {
    ah = (() => {
      if (ah.a - 1 >= ah.h + 2) {
  ...
  function adjToAH(ah: { a: number; h: number; adj: number }) {
    if (ah.adj === 5) {
      ah.h++;
      ah.adj = 0;
    }
  ...
  export function widthSpace(sp: number) {
    if (typeof sp !== "number") {
      throw new TypeError();
    }
    const mod = sp % 11;
