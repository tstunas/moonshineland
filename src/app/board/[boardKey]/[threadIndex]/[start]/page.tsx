...
export default async function ThreadTargetPage({
  params,
}: {
  params: Promise<{ boardKey: string; threadIndex: string; start: string }>;
}) {
  const {
    boardKey,
  ...
  const visiblePosts = posts.filter((post) => post.postOrder === start);
