...
export async function GET(request: NextRequest) {
  ...
  const data = await getPostsPageData(parsePostsQuery(request.nextUrl.searchParams));
...
export async function POST(request: NextRequest) {
  ...
  if (ids.length === 0) {
    return NextResponse.json({ error: "선택된 레스가 없습니다." }, { status: 400 });
  }
  ...
  if (body.value === "visible" || body.value === "hidden") {
    ...
    const summary = `${result.count}개 레스를 ${isHidden ? "숨김" : "공개"} 처리했습니다.`;
  ...
  const summary = `레스 1개의 공개 상태를 ${target.isHidden ? "공개" : "숨김"}으로 변경했습니다.`;
  await recordAdminAudit({
    adminUserId,
    action: "posts-hidden",
    targetType: "post",
