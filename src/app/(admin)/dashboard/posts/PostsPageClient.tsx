...
import type { PostsFilters, PostsPageData } from "@/features/admin/dashboardDetailData";
...
export default function PostsPageClient({ initialData }: { initialData: PostsPageData }) {
  ...
  useEffect(() => {
    syncUrlState("/dashboard/posts", {
      ...
      autoType: filters.autoType !== "all" ? filters.autoType : undefined,
  ...
  const runAction = async (ids: number[], value?: string) => {
    const result = await apiPost<{ summary?: string }>("/api/admin/dashboard/posts", { ids, value });
    if (!result.ok) {
    ...
    <div className="mx-auto w-full max-w-7xl space-y-5">
      <section className="rounded-2xl border border-slate-300 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters(draftFilters); }} className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          <input type="text" value={draftFilters.query} onChange={(event) => setDraftFilters((current) => ({ ...current, query: event.target.value }))} placeholder="작성자/스레드/boardKey 검색" className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <select value={draftFilters.autoType} onChange={(event) => setDraftFilters((current) => ({ ...current, autoType: event.target.value }))} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="all">투하: 전체</option><option value="manual">투하: 수동</option><option value="auto">투하: 자동</option></select>
          <select value={draftFilters.contentType} onChange={(event) => setDraftFilters((current) => ({ ...current, contentType: event.target.value }))} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="all">콘텐츠: 전체</option><option value="text">콘텐츠: text</option><option value="aa">콘텐츠: aa</option><option value="novel">콘텐츠: novel</option><option value="line">콘텐츠: line</option></select>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1120px] text-left text-sm">
            <thead className="bg-slate-100 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">선택</th><th className="px-4 py-3">레스</th><th className="px-4 py-3">속성</th><th className="px-4 py-3">스레드</th><th className="px-4 py-3">생성</th><th className="px-4 py-3">수정</th><th className="px-4 py-3 text-right">개별 작업</th></tr></thead>
            <tbody>
              {data.posts.map((post) => {
                const threadHref = `/board/${post.thread.board.boardKey}/${post.thread.threadIndex}`;
                return <tr key={post.id} className="border-t border-slate-200 align-top"><td className="px-4 py-3"><input type="checkbox" checked={selectedIds.includes(post.id)} onChange={() => setSelectedIds((current) => current.includes(post.id) ? current.filter((id) => id !== post.id) : [...current, post.id])} className="h-4 w-4" /></td><td className="px-4 py-3"><p className="font-semibold text-slate-900">레스 #{post.postOrder}</p><p className="text-xs text-slate-500">작성자 {post.author}</p><p className="text-xs text-slate-500">타입 {post.contentType}</p></td><td className="px-4 py-3"><div className="flex flex-wrap gap-1.5"><span className={`rounded-full px-2 py-0.5 text-xs ${post.isHidden ? "bg-rose-100 text-rose-800" : "bg-emerald-100 text-emerald-800"}`}>{post.isHidden ? "숨김" : "공개"}</span>{post.isAutoPost ? <span className="rounded-full bg-sky-100 px-2 py-0.5 text-xs text-sky-800">자동</span> : <span className="rounded-full bg-slate-200 px-2 py-0.5 text-xs text-slate-700">수동</span>}</div></td><td className="px-4 py-3"><Link href={threadHref} className="font-semibold text-slate-900 hover:text-sky-700">{post.thread.board.name} / #{post.thread.threadIndex}</Link><p className="line-clamp-1 text-xs text-slate-500">{post.thread.title}</p></td><td className="px-4 py-3 text-xs text-slate-500">{formatDateTime(post.createdAt)}</td><td className="px-4 py-3 text-xs text-slate-500">{post.contentUpdatedAt ? formatDateTime(post.contentUpdatedAt) : "-"}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><button type="button" onClick={() => void runAction([post.id])} className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">{post.isHidden ? "공개" : "숨김"}</button><Link href={threadHref} className="rounded-md border border-sky-300 bg-sky-50 px-2 py-1 text-xs font-semibold text-sky-700 hover:bg-sky-100">이동</Link></div></td></tr>;
              })}
            </tbody>
          </table>
        </div>
