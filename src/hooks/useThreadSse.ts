...
export function useThreadSse(
  ...
  useEffect(() => {
    ...
    es.addEventListener(
      "thread:post-content-type-edited",
      (e: MessageEvent) => {
    ...
    // initialReceiveNewPosts는 최초 연결 시에만 의미가 있으므로 deps에서 제외
  ...
  return { connectionId, userCount, setReceiveNewPosts };
}
